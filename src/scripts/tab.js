...
MoInVis.Paracoords.tab = function (parentDiv) {
        ...
        _initHammer = function () {
            ...
            _hammerMan.add(
                new Hammer.Swipe({
                    ...
                    velocity: _hammerSettings.swipeVelocity,
                })
            );
            _hammerMan.add(
                new Hammer.Swipe({
    ...
    this.initVue = function (vueData, vueMethods, vueComputed, vueComponents) {
        ...
        if (vueComponents) {
            for (let componentName in vueComponents) {
                mainApp.component(componentName, vueComponents[componentName]);
            }
        }
        var dataProxy = mainApp.mount(_tabHandle.parentTab.node());
    ...
    this.switchOnSwipeRightEvent = function () {
        _hammerMan.on('swiperight', function (event) {
            event.preventDefault();
            if (self.isEventHandlingInProgress() === false) {
    ...
    this.switchOffEvents = function (onlySwipeEvents = false) {
        _hammerMan.off('swipeleft').off('swiperight');
        if (!onlySwipeEvents) {
            for (let evt in _addedEvents) {
                _hammerMan.off(evt);
            }
