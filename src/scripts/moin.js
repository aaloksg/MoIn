...
MoInVis.Paracoords.moin = function (width, height) {
    var _parentSVG,
        ...
        _init = function () {
            ...
            self.height = height;
            self.id =
                MoInVis.Paracoords.IdStore.moin +
                '_' +
                MoInVis.Paracoords.Count++;

            _parentSVG = document.createElementNS(
                'http://www.w3.org/2000/svg',
                'svg'
            );
            _parentSVG.setAttributeNS(null, 'width', '100%');
            _parentSVG.setAttributeNS(null, 'height', '100%');
            _parentSVG.setAttributeNS(
                null,
                'id',
                self.id + '_' + MoInVis.Paracoords.IdStore.parentSvg
            );

            defs.setAttributeNS(
                null,
                'id',
                self.id + '_' + MoInVis.Paracoords.IdStore.defs
            );
            ...
            self.paracoorder = new MoInVis.Paracoords.paracoorder(
                self,
                paracoordTab,
                d3.select(_parentSVG)
            );
            self.paracoorder.init(0, 0);
            self.paracoorder.draw();

            self.attributeStore = new MoInVis.Paracoords.attributeStore(
                self,
                d3.select('#attrStoreTab'),
                self.paracoorder.axes,
                self.paracoorder.getFocusAndContextSettings()
            );

            self.entryStore = new MoInVis.Paracoords.entryStore(
                self,
                d3.select('#entryStoreTab'),
                self.paracoorder.paths
            );

            self.tabManager = new MoInVis.Paracoords.tabManager(
                self,
                [
                    self.entryStore.getTabHandle(),
                    self.attributeStore.getTabHandle(),
                    self.paracoorder.getTabHandle(),
                ],
                2
            );

            self.brushSetter = new MoInVis.Paracoords.brushSetter(
                self,
                d3.select('#brushSetterOverlay')
            );
            self.tabManager.addOverlayTab(self.brushSetter.getTabHandle());

            self.axisDetailView = new MoInVis.Paracoords.axisDetailView(
                self,
                d3.select('#axisDetailViewOverlay')
            );
