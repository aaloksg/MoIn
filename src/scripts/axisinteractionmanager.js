...
MoInVis.Paracoords.axisInteractionManager = function (
    axis,
    axisGroup,
    axisInnerGroup,
    axisId,
    ...
    this.init = function (brushManager) {
        ...
        _hammerMan.add(
            new Hammer.Press({ event: 'press', pointers: 1, time: 500 })
        );
        _hammerMan.add(
            new Hammer.Swipe({
                event: 'swipe',
    ...
    this.onInteraction = function (event) {
        ...
        if (_paracoorder.checkIfAxesReorderMode()) {
            switch (eventType) {
                case 'panstart':
                    _paracoorder.startAxisReordering(
                        _axis.yPos,
                        _axisId,
                        _axis.indexInVisibilityArray,
                        _axis.height
                    );
                    break;

                case 'pan':
                    _paracoorder.reorderAxis(
                        event.deltaY,
                        _axis.indexInVisibilityArray,
                        _axis.indexInGlobalArray
                    );
                    break;

                case 'panend':
                    _paracoorder.stopAxisReordering(
                        _axis.indexInVisibilityArray
                    );
                    break;

                case 'swipeleft':
                    _paracoorder.removeAxis(
                        _axis.indexInVisibilityArray,
                        _axis.indexInGlobalArray,
                        true
                    );
            ...
            switch (eventType) {
                ...
                case 'singletap':
                    if (targetId.includes('LabelText', 0)) {
                        _paracoorder.moin.axisDetailView.setUpData(
                            _axis.attribute,
                            _axis.attributeLabel,
                            _paracoorder.paths
                        );
