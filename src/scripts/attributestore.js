...
MoInVis.Paracoords.attributeStore = function (
    moin,
    parentDiv,
    axes,
    ...
    var self = this,
            ...
            _sortable = Sortable.create(simpleList, {
                ...
                onEnd: function (evt) {
                    if (evt.newIndex !== evt.oldIndex) {
                        ...
                        _addAction(_undoReorder, undefined, [
                            _axes.map((axis) => axis.attribute),
                        ]);

                        order.forEach((name, index) => {
                            _vueData.axesArray.splice(
                                index,
                                0,
                                _vueData.axesArray.splice(
                                    _vueData.axesArray.findIndex(
            ...
            interact('.draggable').draggable({
                ...
                listeners: {
                    ...
                    move(event) {
                        ...
                        event.target.style.transform = `translate(${_focusPanelStartPosition.x}px, ${_focusPanelStartPosition.y}px)`;
                    },
                    end(event) {
                        var newFI;
                        d3.select('.focusPanelBar')
                        ...
                        newFI = Math.round(
                            _focusPanelStartPosition.y / _snapHeight()
                        );
                        if (newFI < 0) {
                            newFI = 0;
                        }
                        if (
                            newFI + _vueData.focusContextSettings.axesInFocus >
                            _vueData.notSortableIndexFrom
                        ) {
                            newFI =
                                _vueData.notSortableIndexFrom -
                                _vueData.focusContextSettings.axesInFocus;
        ...
        _boxWidth = function () {
            ...
            return {
                short: paddingLeft + attrHandleBox.clientWidth,
                long: attrTextBox.clientWidth + attrHandleBox.clientWidth,
            };
        },
        _boxHeight = function () {
        ...
        _decreaseNumber = function () {
            ...
            if (
                this.focusContextSettings.axesInFocus <=
                    this.focusContextSettings.maxAxesInFocus &&
                this.focusContextSettings.axesInFocus >
                    this.focusContextSettings.minAxesInFocus
            ) {
                _setNumberAxesInFocus(
        ...
        _check = function (e, axis, index) {
            ...
            if (axis.visible) {
                ...
                if (index >= this.notSortableIndexFrom) {
                    //checked item within unckecked items
                    ...
                    _addAction(_undoVisibility, this, [
                        axis,
                        !axis.visible,
                        axisIndex,
                    ]);
                ...
                if (
                    ...
                    //can't uncheck item when reach minAxesInFocus
                    axis.setVisibility(!axis.visible);
                } else {
                    //remove the axis from array
                    axisIndex = this.axesArray.indexOf(axis);
                    ...
                    if (
                        this.focusContextSettings.focusIndex +
                            this.focusContextSettings.axesInFocus >
                        this.notSortableIndexFrom
                    ) {
                        _setFocusIndex(
        ...
        _setNumberAxesInFocus = function (numberAxesInFocus) {
            _addAction(_undoNumberAxesInFocus, undefined, [
                _vueData.focusContextSettings.axesInFocus,
            ]);
        ...
        _setFocusIndex = function (focusIndex) {
            _addAction(_undoFocusIndex, undefined, [
                _vueData.focusContextSettings.focusIndex,
            ]);
        ...
        _adjustFocusPanel = function () {
            _focusPanelStartPosition.y =
                _focusContextSettings.focusIndex * _snapHeight();
