...
MoInVis.Paracoords.axis = function (
    axisParent,
    id,
    attributeProps,
    attrScale,
    ...
    this.getBrushConfigurations = function () {
        ...
        brushes.forEach((brush, index) => {
            ...
            brush.range = [
                Math.floor(_attrScale.invert(brush.range[0])),
                Math.floor(_attrScale.invert(brush.range[1])),
            ];
    ...
    this.draw = function (xPos, yPos) {
        ...
        _axisInnerGroup.call(
            (_axis = d3.axisBottom(_attrScale).ticks(4, '~s').tickSize(15))
        );

        // Tick styling.
        _axisInnerGroup.selectAll('g.tick').select('text').attr('y', 22);
        _axisInnerGroup.selectAll('g.tick').select('line').attr('y2', 10);
        ...
        const measuredSize = _textInnerGroup.select('text').node().getBBox();
        _textRect
    ...
    this.setY = function (newY) {
        this.yPos = newY;
        _axisGroup.attr('transform', 'translate(0,' + newY + ')');
    };
    ...
    this.setXY = function (newX, newY) {
        this.yPos = newY;
        this.xPos = newX;
        _axisGroup.attr('transform', 'translate(' + newX + ',' + newY + ')');
    };
    ...
    this.startWiggling = function () {
        ...
        _wigglingIntervalId = setInterval(function () {
            ...
            _textInnerGroup.attr(
                'transform',
                'rotate(' + y + ',' + _textGroupCenter + ',0)'
            );
            seed = seed + speed;
        }, 10);
    };
