...
MoInVis.Paracoords.brushSetter = function (moin, parentDiv) {
    ...
    var self = this,
        ...
        _labelClick = function (event, item) {
            if (
                ...
                event.target.className.indexOf('modal') === -1 &&
        ...
        _enableDisableBrush = function (item) {
            self.moin.paracoorder.enableDisableBrush(
                item.axisId,
                item.brushId,
                item.active
            );
        ...
        _incrementValue = function () {
            var stepVal = Math.floor(
                    (this.brushprops.axisRange[1] -
                        this.brushprops.axisRange[0]) *
                        _incDecPC
                ),
        ...
        _updateCopy = function (item) {
            var brush = _brushesCopy.find(
                (brush) => brush.axisName === item.axisName
            );
            if (brush) {
                brush.range[0] = item.range[0];
                brush.range[1] = item.range[1];
                brush.rangeText[0] = item.rangeText[0] =
                    MoInVis.Paracoords.util.format(item.range[0]);
                brush.rangeText[1] = item.rangeText[1] =
                    MoInVis.Paracoords.util.format(item.range[1]);
            }
        },
        ...
        _onKeyInput = function (event) {
            ...
            if (
                this.brushprops.axisRange[0] <= newVal &&
                newVal <= this.brushprops.axisRange[1]
            ) {
                if (this.startValueActive) {
                    if (newVal !== brushEnd) {
                        if (newVal > brushEnd) {
        ...
        _applyModalChanges = function () {
            ...
            self.moin.paracoorder.setBrushValueRange(
                brushProps.axisId,
                brushProps.brushId,
                brushProps.range
            );
            if (
                brushProps.range[0] === brushProps.axisRange[0] &&
                brushProps.range[1] === brushProps.axisRange[1]
            ) {
    ...
    this.onTabActivated = function () {
        ...
        brushConfig.forEach((config, index) => {
            _vueData.brushes[index] = config;
        });
