...
MoInVis.Paracoords.paracoorder = function (moin, parentDiv, svgParent) {
    ...
    var self = this,
        ...
        _visibleAxes = [],
        _axisHeight,
        _focusAndContextSettings = {
            // Settings for focus and context.
            focusIndex: 1,
            axesInContext: 2,
            axesInFocus: 3,
            maxAxesInFocus: 5,
        ...
        _positionProps = {
            // Positions for our content, including texts.
            top: _margins.top,
            left: _margins.left,
        ...
        _innerPositionProps = {
            // Positions for our visualizations.
            top: _positionProps.top + _innerMargins.top,
            left: _positionProps.left + _innerMargins.left,
            width:
                _positionProps.width - _innerMargins.left - _innerMargins.right,
            height:
                _positionProps.height -
                _innerMargins.top -
                _innerMargins.bottom,
            hideAtTop: -0.5 * _positionProps.height,
            hideAtBottom:
                _positionProps.top +
                _innerMargins.top +
                1.5 * _positionProps.height,
        },
        _clipRect,
        _lastPosY = 0,
        _lastDeltaY = 0,
        ...
        _createStyles = function () {
            var defs = document.getElementById(
                    self.moin.id + '_' + MoInVis.Paracoords.IdStore.defs
                ),
                linearGrad,
                stopElement;
            ...
            linearGrad.setAttributeNS(
                null,
                'id',
                MoInVis.Paracoords.IdStore.TopCIGradNormal
            );
            ...
            linearGrad.setAttributeNS(
                null,
                'id',
                MoInVis.Paracoords.IdStore.TopCIGradPressed
            );
            linearGrad.setAttributeNS(null, 'x1', '0%');
            linearGrad.setAttributeNS(null, 'x2', '0%');
            linearGrad.setAttributeNS(null, 'y1', '0%');
            ...
            linearGrad.setAttributeNS(
                null,
                'id',
                MoInVis.Paracoords.IdStore.BottomCIGradNormal
            );
            linearGrad.setAttributeNS(null, 'x1', '0%');
            linearGrad.setAttributeNS(null, 'x2', '0%');
            linearGrad.setAttributeNS(null, 'y1', '0%');
            ...
            linearGrad.setAttributeNS(
                null,
                'id',
                MoInVis.Paracoords.IdStore.BottomCIGradPressed
            );
            linearGrad.setAttributeNS(null, 'x1', '0%');
            linearGrad.setAttributeNS(null, 'x2', '0%');
            linearGrad.setAttributeNS(null, 'y1', '0%');
        ...
        _calculateAxisSpacing = function () {
            ...
            if (_focusIndex < _axesInTopContext) {
                ...
                _focusIndex + _axesInFocus >
                axisCount - _axesInBottomContext
            ) {
            ...
            numAxesInViewport =
                _axesInFocus + _axesInTopContext + _axesInBottomContext; // Total number of axes currently displayed.
            unusedSpace =
                _innerPositionProps.height - numAxesInViewport * _axisHeight;
            ...
            if (_axesInFocus > 1) {
                ...
                gapUnits +=
                    (_axesInFocus - 1) *
                    _focusAndContextSettings.extraGapFactor;
            }
            contextAxisGap = unusedSpace / gapUnits;
            _focusAndContextSettings.contextAxisOverflow =
                contextAxisGap *
                _focusAndContextSettings.contextAxisOverflowFactor;
            _focusAndContextSettings.pinchAxisOverflow = contextAxisGap;
            focusAxisGap =
                contextAxisGap * _focusAndContextSettings.extraGapFactor;

            // Set positions for hidden axes depending on the gap between context axes.
            _innerPositionProps.hideAtTop = -4 * contextAxisGap;
            _innerPositionProps.hideAtBottom =
                _positionProps.top +
                _innerMargins.top +
                _positionProps.height +
                4 * contextAxisGap;
        ...
        _rearrangeAxes = function () {
            let i,
                length =
                    _axesInFocus + _axesInTopContext + _axesInBottomContext,
        ...
        _resetAxesRanges = function () {
            ...
            self.axes.forEach(function (axis) {
                extent = d3.extent(
                    items.map((region) =>
                        self.paths[region].visible
                            ? self.paths[region].data[_chosenYear][
                                  axis.attribute
                              ]
                            : null
                    )
        ...
        _shiftAxesByPixels = function (pixels) {
            let length = _axesInFocus + _focusIndex + _axesInBottomContext,
                ...
                focusEndPos =
                    _axesPositions[_axesInTopContext + _axesInFocus - 1];

            if (pixels < 0) {
                // Shifting axes upwards.
                if (length < _visibleAxes.length - 1) {
                    // Include the axis outside viewport.
                    length++;
                } else if (length === _visibleAxes.length) {
                    // If last axis is reached, do not move the last axis.
                    length--;
                }
                if (_axesInTopContext === 0) {
                    // Do not move first axis until top context area contains axes.
                    i++;
                }
                for (i; i < length; i++) {
                    axis = _visibleAxes[i];
                    ...
                    if (axis.yPos > focusStartPos && axis.yPos <= focusEndPos) {
                        ...
                        axis.setY(
                            axis.yPos +
                                pixels * _focusAndContextSettings.extraGapFactor
                        );
                    } else {
                ...
                // Shifting axes downwards.
                if (_axesInBottomContext === 0) {
                    // Do not move last axis until bottom context area contains axes.
                    length--;
                }
                if (i === 0) {
                    // If first axis is reached, do not move the first axis.
                    i++;
                } else if (i > 1) {
                    // Include the axis outside viewport.
                    i--;
        ...
        _squeezeAxesByPixels = function (pixels) {
            let length, i, axis, lastVisibleAxisIndex;

            if (_axesInBottomContext) {
                // If there are axes in the context below.
                ...
                if (length < _visibleAxes.length - 1) {
                    // Include the axis outside viewport, if available.
                    length++;
                } else if (length === _visibleAxes.length) {
                    // If last axis is reached, do not move it.
                    length--;
                }
                if (i === 0) {
                    // If first axis is reached, do not move the first axis.
        ...
        _panUpDown = function (event) {
            if (self.pinchScrollInProgress) {
                ...
                if (
                    Date.now() - _pinchScrollProps.lastAxisRearrangement >
                    _pinchScrollProps.transitionSpeed
                ) {
                    if (deltaY < 0) {
                        if (_focusIndex < _visibleAxes.length - _axesInFocus) {
                            ...
                            if (
                                _pinchScrollProps.overflowY <=
                                -_focusAndContextSettings.contextAxisOverflow
                            ) {
                                _pinchScrollProps.overflowY = 0;
                                // Set the tranition speed to scrolling transition speed.
                                MoInVis.Paracoords.TransitionSpeed =
                                    _pinchScrollProps.transitionSpeed;
                                _setFocusIndex(_focusIndex + 1);
                                _pinchScrollProps.lastAxisRearrangement =
                                    Date.now();
                                // Reset the transition speed to normal.
                                MoInVis.Paracoords.TransitionSpeed =
                                    MoInVis.Paracoords.NormalTransitionSpeed;
                            } else {
                                // If overflow is not exceeded, shift axes by delta pixels.
        ...
        _onPinchStart = function (event) {
            ...
            _pinchScrollProps.initialDistance = Math.abs(
                pointer1.y - pointer2.y
            ); // Initial distance between the pointers.
            _setPinchScrollInProgress();
        },
        ...
        _onPinch = function (event) {
            ...
            if (
                ...
                if (overflowY > 0) {
                    // Pinch-in
                    if (
                        _axesInFocus < _focusAndContextSettings.maxAxesInFocus
                    ) {
                        if (
                            _pinchScrollProps.overflowY >
                            _focusAndContextSettings.pinchAxisOverflow
                        ) {
                            _pinchScrollProps.overflowY = 0;
                    ...
                    // Pinch-out
                    if (
                        _axesInFocus > _focusAndContextSettings.minAxesInFocus
                    ) {
                        if (
                            _pinchScrollProps.overflowY <
                            -_focusAndContextSettings.pinchAxisOverflow
        ...
        _onPinchStopped = function () {
            if (_pinchScrollProps.timer) {
                _pinchScrollProps.timer.restart(
                    _resetPinchScrollInProgress,
                    250
                );
            } else {
                _pinchScrollProps.timer = d3.interval(
                    _resetPinchScrollInProgress,
                    250
                );
            }
    ...
    this.init = function (x, y) {
        ...
        _paracoordHolder = _parentGroup
            ...
            .attr(
                'clip-path',
                'url(#' + MoInVis.Paracoords.IdStore.paracoordClipper + ')'
            )
    ...
    this.draw = function () {
        ...
        attributes.forEach(function (attr) {
            extent = d3.extent(
                items.map((region) =>
                    self.paths[region].visible
                        ? self.paths[region].data[_chosenYear][attr.prop]
                        : null
                )
            );
            _attrScales.set(
                ...
                d3
                    .scaleLinear()
                    .range([
                        _innerPositionProps.left,
                        _innerPositionProps.left + _innerPositionProps.width,
                    ])
                    .domain(extent)
            );
        });
        ...
        _topCI = new MoInVis.Paracoords.contextIndicator(
            _parentGroup,
            _positionProps.left,
            0,
            _ciSize,
            _positionProps.width,
            'TopCI',
            true,
            _quickScrollUp
        );
        _bottomCI = new MoInVis.Paracoords.contextIndicator(
            _parentGroup,
            _positionProps.left,
            _height - _ciSize,
            _ciSize,
    ...
    this.drawAttributeAxes = function () {
        ...
        for (i = 0; i < length; i++) {
            axis = new MoInVis.Paracoords.axis(
                _axisParentGroup,
                _id + '_Year_' + _chosenYear,
                attributes[i],
                _attrScales.get(attributes[i].prop),
                this
            );
    ...
    this.resize = function () {
        ...
        _innerMargins.left =
            ...
            _innerMargins.bottom =
                _width * 0.06;
        _innerPositionProps.width =
            _positionProps.width - _innerMargins.left - _innerMargins.right;
        _innerPositionProps.height =
            _positionProps.height - _innerMargins.top - _innerMargins.bottom;
    ...
    this.drawPaths = function () {
        let regions = MoInVis.Paracoords.Data.itemsForWaste,
            ...
            colourBlindSafe =
                this.moin.colourScheme ===
                MoInVis.Paracoords.ColourScheme.ColourblindSafe;
        _pathParentGroup = _paracoordHolder
            .append('g')
            .attr('id', _id + '_PathParentGrp');

        for (i = 0; i < length; i++) {
            this.paths[regions[i]] = new MoInVis.Paracoords.itemPath(
                _pathParentGroup,
                _id,
                regions[i],
                this
            );
            this.paths[regions[i]].init(
                wasteByCountries[regions[i]],
                _chosenYear,
                getColour(i, colourBlindSafe)
            );
            this.paths[regions[i]].draw();
        }
        this.paths[
            'European_sp_Union_sp_-_sp_28_sp_countries_sp__ob_2013-2020_cb_'
        ].setVisibility(false);
        this.paths[
            'European_sp_Union_sp_-_sp_27_sp_countries_sp__ob_from_sp_2020_cb_'
        ].setVisibility(false);
    };
    ...
    this.getPathPointsInfo = function (itemName) {
        let i = 0,
            ...
            value,
            data =
                MoInVis.Paracoords.Data.wasteByCountries[itemName].data[
                    _chosenYear
                ];
    ...
    this.removeAxis = function (index, indexGlobal, swipeLeft) {
        ...
        if (_visibleAxes.length > _focusAndContextSettings.minAxesInFocus) {
            ...
            self.axes.splice(
                self.axes.findIndex((axis) => axis === _visibleAxes[index]),
                1
            );
    ...
    this.brushPaths = function () {
        ...
        for (path in this.paths) {
            ...
            if (item.visible) {
                data =
                    MoInVis.Paracoords.Data.wasteByCountries[item.itemName]
                        .data[_chosenYear];
                for (axisIndex = 0; axisIndex < length; axisIndex++) {
                    ...
                    if (value !== null && value !== '') {
                        if (
                            axis.checkPathBrushed(axis.getXY(value)[0]) ===
                            false
                        ) {
    ...
    this.enterAxesReorderMode = function () {
        ...
        if (this.pinchScrollInProgress === false) {
            // Only enter axes reorder mode when pinch and scroll are not in progress.
