...
MoInVis.Paracoords.axisBrush = function (brushParent, id, brushHeight) {
    ...
    this.setVisibility = function (visible) {
        ...
        if (this.visible) {
            _brushGroup.style('display', 'inherit').attr('opacity', 1);
        } else {
            _brushGroup.attr('opacity', 0).style('display', 'none');
    ...
    this.setColour = function (colour) {
        ...
        if (_brushRect) {
            _brushRect.attr('stroke', _brushColour).attr('fill', _brushColour);
        }
        if (_handleStart) {
            _handleStart
    ...
    this.setBrushStart = function (brushStart) {
        ...
        _handleEndGrp.attr(
            'transform',
            'translate(' + _brushEnd + ',' + -_brushHeight / 2 + ')'
        );
        _handleStartGrp.attr(
            'transform',
            'translate(' + _brushStart + ',' + _brushHeight / 2 + ')'
        );
    };
    ...
    this.setBrushEnd = function (brushEnd) {
        ...
        if (_brushEnd < _brushStart) {
            ...
            _brushRect.attr('width', _brushEnd - _brushStart);
            _tapCapturer.attr('width', _brushEnd - _brushStart);
        }
        _handleEndGrp.attr(
            'transform',
            'translate(' + _brushEnd + ',' + -_brushHeight / 2 + ')'
    ...
    this.onBrushingEnd = function () {
        ...
        _handleStartGrp.style('display', 'inherit').attr('opacity', 1);
        _handleEndGrp.style('display', 'inherit').attr('opacity', 1);
        ...
        if (_handleVisibility) {
            if (gestureOrigin.y <= _handleGestureChecker.topHBottom) {
                ...
                if (gestureOrigin.y >= _handleGestureChecker.topHTop) {
                    leftBound =
                        _brushEnd - _handleGestureChecker.leftRightMargin;
                    rightBound =
                        _brushEnd + _handleGestureChecker.leftRightMargin;
                    if (
                        gestureOrigin.x >= leftBound &&
                        gestureOrigin.x <= rightBound
                    ) {
                        _ehInteraction = true;
                        _stopTimer();
                ...
                if (gestureOrigin.y <= _handleGestureChecker.bottomHBottom) {
                    leftBound =
                        _brushStart - _handleGestureChecker.leftRightMargin;
                    rightBound =
                        _brushStart + _handleGestureChecker.leftRightMargin;
                    if (
                        gestureOrigin.x >= leftBound &&
    ...
    this.isInterfering = function (probableHandlePos) {
        ...
        return !(
            probableHandlePos[1] < brushBounds[0] ||
            probableHandlePos[0] > brushBounds[1]
        );
    };
