...
MoInVis.Paracoords.brushManager = function (axisId, attrScale, paracoorder) {
    ...
    var self = this,
        ...
        _isValidMove = function () {
            var length = _activeBrushes.length,
                i,
                validMove = true,
                probableHandlePos,
                range = _attrScale.range();
            ...
            if (length > 1) {
                probableHandlePos =
                    _activeBrush.getProbableHandlePos(_gesturePos);
                for (i = 0; i < length; i++) {
                    if (_activeBrushes[i] !== _activeBrush) {
                        if (
                            _activeBrushes[i].isInterfering(probableHandlePos)
                        ) {
                            // [TODO]: Add robustness, so handle positions update to best possible change.
                            validMove = false;
                            break;
                        }
                    }
        ...
        _tryToDeactivateBrush = function () {
            var range = _attrScale.range(),
                brushRange = _activeBrush.getBrushBounds();
    ...
    this.enableDisableBrush = function (brushId, active, doNotBrushPaths) {
        ...
        if (active) {
            brushIndex = _inactiveBrushes.findIndex(
                (item) => item.id === brushId
            );
            if (brushIndex > -1) {
            ...
            brushIndex = _activeBrushes.findIndex(
                (item) => item.id === brushId
            );
            if (brushIndex > -1) {
    ...
    this.getBrushValueRanges = function () {
        ...
        _brushes.forEach((brush) => {
            valueRanges.push(
                brush.getBrushBounds().map((bound) => _attrScale.invert(bound))
            );
        });
        return valueRanges;
    };
    ...
    this.setBrushValueRanges = function (valueRanges) {
        ...
        valueRanges.forEach((valueRange, index) => {
            ...
            if (brushRange[0] >= axisRange[1]) {
                disableBrush = true;
                brushRange[0] =
                    axisRange[1] - (axisRange[1] - axisRange[0]) / 10;
                brushRange[1] = axisRange[1];
            } else if (brushRange[1] <= axisRange[0]) {
                disableBrush = true;
                brushRange[1] =
                    axisRange[0] + (axisRange[1] - axisRange[0]) / 10;
        ...
        if (_paracoorder.pinchScrollInProgress) {
            ...
            if ((length = _activeBrushes.length) > 0) {
                ...
                for (let i = 0; i < length; i++) {
                    if (
                        _activeBrushes[i].checkHandleInteraction(gestureOrigin)
                    ) {
            ...
            if (createNewBrush) {
                if (_activeBrushes.length === 0) {
                    // [Important]: Remove check to handle drawing of multiple brushes.
                    ...
                    if (_inactiveBrushes.length > 0) {
                        ...
                        _activeBrush = new MoInVis.Paracoords.axisBrush(
                            _brushParent,
                            _axisId + '_Brush_' + _brushes.length,
                            _brushHeight
                        );
    ...
    this.onTap = function (event) {
        var id, index, deactivateBrushes;
        if (_activeBrushes.length > 0) {
            deactivateBrushes = true;
            id = event.changedPointers[0].target.id;
            if (id) {
                index = event.changedPointers[0].target.id.split(
                    _axisId + '_Brush_'
                )[1];
                if (index) {
