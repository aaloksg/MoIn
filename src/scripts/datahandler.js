...
MoInVis.Paracoords.dataHandler = (function () {
    var self = this,
        ...
        _extractData = function (data, propertyName, propertyText) {
            ...
            MoInVis.Paracoords.Data.wasteAttributes.push({
                prop: propertyName,
                text: propertyText,
            });
            for (i = 0; i < length; i++) {
                ...
                regionId = regionText
                    ...
                    .replace(/\"/g, '_dq_');
                if (
                    MoInVis.Paracoords.Data.wasteByCountries[regionId] ===
                    undefined
                ) {
                    MoInVis.Paracoords.Data.wasteByCountries[regionId] =
                        newObject = {
                ...
                for (key in object) {
                    if (key !== 'GEO' && key !== '') {
                        ...
                        if (
                            MoInVis.Paracoords.Data.yearsForWaste.indexOf(
                                year
                            ) === -1
                        ) {
                ...
                if (itemValid && validItems.indexOf(regionId) === -1) {
                    // If item has at least one valid entry.
        ...
        _initData = function () {
            ...
            _extractData(
                MoInVis.Paracoords.Data.totalWaste,
                'totalWaste',
                'Total Waste'
            );
            _extractData(
                MoInVis.Paracoords.Data.primaryWaste,
                'primaryWaste',
                'Primary Waste'
            );
            _extractData(
                MoInVis.Paracoords.Data.secondaryWaste,
                'secondaryWaste',
                'Secondary Waste'
            );
            _extractData(
                MoInVis.Paracoords.Data.rubberWaste,
                'rubberWaste',
                'Rubber Waste'
            );
            _extractData(
                MoInVis.Paracoords.Data.glassWaste,
                'glassWaste',
                'Glass Waste'
            );
            _extractData(
                MoInVis.Paracoords.Data.healthCareBioWaste,
                'healthCareBioWaste',
                'HC/Bio Waste'
            );
            _extractData(
                MoInVis.Paracoords.Data.plasticWaste,
                'plasticWaste',
                'Plastic Waste'
            );
            _extractData(
                MoInVis.Paracoords.Data.chemWaste,
                'chemWaste',
                'Chemical Waste'
            );
            _extractData(
                MoInVis.Paracoords.Data.paperCardboardWaste,
                'paperCardboardWaste',
                'Paper & Cardboard Waste'
            );
            _extractData(
                MoInVis.Paracoords.Data.textileWaste,
                'textileWaste',
                'Textile Waste'
            );
            _extractData(
                MoInVis.Paracoords.Data.discardedVehicles,
                'discardedVehicles',
                'Discarded Vehicles'
            );
            _extractData(
                MoInVis.Paracoords.Data.animalAndMixedFoodWaste,
                'animalAndMixedFoodWaste',
                'Animal & Mixed Food Waste'
            );
            _extractData(
                MoInVis.Paracoords.Data.animalFaecesUrineAndManure,
                'animalFaecesUrineAndManure',
                'Animal Faeces, Urine & Manure'
            );
            _extractData(
                MoInVis.Paracoords.Data.vegetalWaste,
                'vegetalWaste',
                'Vegetal Waste'
            );
            _extractData(
                MoInVis.Paracoords.Data.householdAndSimilarWaste,
                'householdAndSimilarWaste',
                'Household & Similar Waste'
            );
            _extractData(
                MoInVis.Paracoords.Data.metalWasteFerrousAndNon,
                'metalWasteFerrousAndNon',
                'Metal Waste (Ferrous & Non)'
            );
            _extractData(
                MoInVis.Paracoords.Data.woodWaste,
                'woodWaste',
                'Wood Waste'
            );
